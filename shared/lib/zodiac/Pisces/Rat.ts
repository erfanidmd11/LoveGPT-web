import { ZodiacNarrative } from '../../types';

export const Pisces_Rat_male: ZodiacNarrative = {
summary: "A dreamy strategist — emotionally deep, intellectually agile, quietly loyal, and fiercely devoted to a love that feels soulful, stable, and *real*.",
 energy: "Imaginative + Strategic",
 dominantElement: "Water + Earth",
 keywords: [
 'intuitive', 'clever', 'emotionally deep', 'loyal',
 'romantic', 'strategic', 'creative', 'reserved'
 ],
  fullNarrative: `
You are intuition with a plan — poetic yet calculated, gentle yet discerning, and quietly magnetic in your depth. You live in the realm of possibility but move through the world with intention. You don't just dream — you *design*.

In love, you're romantic, emotionally intelligent, and observant. You crave a connection that's equal parts heart and mind. You want a partner who's emotionally open, intellectually curious, and respectful of your sensitivity without *trying to fix it*. You offer soulful presence, but only where there's *substance*.

You love through attentiveness, loyalty, and subtle devotion. You don't always announce your feelings — you show them in small, meaningful ways. But if your emotional generosity is manipulated or your trust broken? You don't rage. You *release* — quietly, and permanently.

In work, you're strategic and idealistic — a rare blend of visionary and tactician. You're drawn to purpose-driven paths that let your creativity and empathy coexist. You may move quietly, but your impact runs deep.

In friendships and family, you are loyal, thoughtful, and often the emotional anchor. You pick up what others miss. You offer safety without fanfare. You may not always feel understood — but your love is never in question to those who *matter*.

---

🧭 **Your Energetic Compass** (ARIA's Insight Map)

✨ What You Shine At:
- Loving with quiet devotion and emotional nuance.
- Building connection that is soulful, safe, and steady.
- Inspiring trust through subtle insight and loyalty.
- Protecting what matters without seeking control.
- Blending imagination with grounded emotional presence.

⚡ Growth Points:
- You hide emotional needs behind competence or silence.
- You expect others to intuit your feelings.
- You give depth, but struggle to ask for depth in return.
- You fear being misunderstood, so you self-edit your truth.

🌱 What's Waiting to Bloom:
- Trusting that asking for more doesn't make you "too much."
- Letting someone love you *before* you've perfectly explained yourself.
- Sharing your needs without apology or over-analysis.
- Believing that your emotional depth is a gift — not a liability.

🚨 Watch For:
- Staying in "almost right" relationships out of comfort or hope.
- Avoiding conflict to preserve harmony — even when it stifles connection.
- Holding back your truth to avoid being misunderstood.
- Over-functioning to feel emotionally secure.

`
};
export const Pisces_Rat_female: ZodiacNarrative = {
summary: "A sensitive strategist — emotionally deep, quietly intuitive, fiercely loyal, and deeply devoted to a love that feels both safe and soul-stirring.",
 energy: "Soft + Intelligent",
 dominantElement: "Water + Earth",
 keywords: [
 'intuitive', 'romantic', 'emotionally aware', 'strategic',
 'loyal', 'gentle', 'observant', 'private'
 ],
  fullNarrative: `
You are dream wrapped in discipline — emotionally layered, deeply intuitive, and intellectually sharp beneath your soft exterior. You don't just feel deeply — you *understand* deeply. You crave connection that honors both your tenderness and your intelligence.

In love, you are romantic, reserved, and richly emotional. You give with quiet devotion, but only to those who move with care. You want a partner who sees past your sweetness and into your *strategy* — someone who respects your emotional world without trying to manage or define it.

You love through gestures, presence, and intuitive care. You remember the details, sense the shifts, and hold space without needing credit. But if your trust is broken or your softness is mistaken for weakness? You retreat. *Entirely*. With dignity and a silence that speaks louder than explanation.

In work, you're thoughtful, creative, and quietly powerful. You blend vision with caution — moving when it feels right, not just when it looks right. You're not made for loud rooms, but when you speak, people *listen*.

With friends and family, you're loyal, nurturing, and profoundly empathetic. You absorb more than you show, love deeper than you admit, and are often the quiet emotional compass for those around you. You give — but you're learning to give *without losing yourself*.

---

🧭 **Your Energetic Compass** (ARIA's Insight Map)

✨ What You Shine At:
- Loving with intention, tenderness, and intuitive depth.
- Seeing others fully — and honoring what they hide.
- Offering care that's both gentle and wise.
- Creating calm where others bring chaos.
- Making emotion feel safe, not overwhelming.

⚡ Growth Points:
- You wait to be chosen before you reveal your full self.
- You downplay your needs to avoid appearing "difficult."
- You offer emotional depth before ensuring it's safe to do so.
- You retreat quietly when hurt — instead of speaking your truth.

🌱 What's Waiting to Bloom:
- Trusting that your softness is a strength, not a risk.
- Asking for what you need without apology or fear.
- Letting someone witness your rawness — not just your grace.
- Releasing the need to be "perfectly low maintenance" to be loved.

🚨 Watch For:
- Over-giving in emotionally uneven relationships.
- Settling for peace over passion.
- Confusing emotional safety with emotional suppression.
- Shrinking your truth to preserve someone else's comfort.

`
};