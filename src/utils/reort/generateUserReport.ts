// src/utils/report/generateUserReport.ts
import { OpenAI } from 'openai';
import { UserResume } from '@/types/UserResume';

const openai = new OpenAI({ apiKey: process.env.OPENAI_API_KEY! });

export async function generateUserReport(resume: UserResume): Promise<{ summary: string; readinessScore: number }> {
  const traits = [
  ...
  // Readiness score calculation (basic for now)
  ...
  ].filter(Boolean).length * 14; // max ~100
