...
export default function ParenthoodStep2() {
  const router = useRouter();
  const [selected, setSelected] = useState<string | null>(null);

  const handleNext = () => {
    if (selected) {
      localStorage.setItem('parenthood-step2', selected);
      router.push('/dashboard/personality/parenthood/step-3');
    }
  };
    ...
    <DashboardLayout>
      <div className="max-w-2xl mx-auto py-12 px-4">
        ...
        <h2 className="text-lg font-medium text-gray-800 mb-4">
          2. What role do you see yourself playing in raising a child?
        </h2>
        <div className="space-y-4">
          {[
            ...
            'Prefer not to be involved day-to-day'
          ].map((option) => (
            <button
              key={option}
