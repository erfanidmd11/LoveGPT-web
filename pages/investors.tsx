  ...
  return (
    <MainLayout>
      <div className="flex bg-gradient-to-b from-white to-indigo-100 text-gray-800 font-sans">
        ...
        <aside className="hidden md:block w-64 p-6 sticky top-0 h-screen overflow-y-auto bg-white border-r border-gray-200">
          ...
          <ul className="space-y-2 text-base">
            {sections.map((section, idx) => (
              <li key={idx}>
                <a href={`#section-${idx}`} className="hover:text-indigo-600 transition-all duration-300" onClick={(e) => {
                  e.preventDefault();
        ...
        {/* Main Content */}
        <main className="flex-1 px-6 md:px-16 pt-0 space-y-20">
          ...
          {sections.map((section, idx) => (
            <section key={idx} id={`section-${idx}`} className="scroll-mt-24">
              ...
              <div className="text-lg text-gray-800 leading-relaxed bg-white p-6 rounded-xl shadow-md" dangerouslySetInnerHTML={{ __html: sectionContent[idx] }} />
            </section>
          ))}
          ...
          <section className="py-20 text-center border-t border-gray-300">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-[1.618rem] max-w-[1440px] mx-auto">
              ...
              <Button className="w-full aspect-[1.618]" variant="outline" onClick={() => window.open('/financials', '_blank')}>
                <BarChart className="mr-2" /> Financials
              </Button>
              <Button className="w-full" variant="outline" onClick={() => window.open('/pitch-deck', '_blank')}>
