...
const mockUserA = {
  name: 'You',
  zodiac: typeof window !== 'undefined' ? localStorage.getItem('onboardingZodiac') || 'Leo' : 'Leo',
  personality: 'INFJ',
...
const calculateCompatibility = (a, b) => {
  ...
  // Zodiac match bonus
  if (
    zodiacProfiles[a.zodiac]?.bestMatches.includes(b.zodiac)
  ) {
    score += 20;
    breakdown.push('Zodiac signs are highly compatible 🔮');
  }

  // Personality type variation
    ...
    <>
      ...
      <div className="min-h-screen bg-gradient-to-br from-pink-50 to-purple-100 p-6">
        <div className="max-w-2xl mx-auto bg-white rounded-2xl shadow-xl p-8">
          <h1 className="text-2xl font-bold text-pink-600 mb-6">Compatibility Snapshot 💞</h1>

          <div className="grid grid-cols-2 gap-6 text-center mb-6">
            {[mockUserA, mockUserB].map((user, idx) => (
              <div key={idx}>
                <h2 className="font-semibold text-purple-700">{user.name}</h2>
                <p className="text-sm">Zodiac: <strong>{user.zodiac}</strong></p>
                <p className="text-sm">Personality: <strong>{user.personality}</strong></p>
          ...
          <div className="text-center mb-6">
            ...
            <p className="text-sm text-gray-500 mt-2">Powered by zodiac, personality & core values</p>
          </div>

          <div className="bg-purple-50 border border-purple-200 rounded-xl p-4">
            <h4 className="text-md font-semibold text-purple-600 mb-2">ARIA’s Insights</h4>
